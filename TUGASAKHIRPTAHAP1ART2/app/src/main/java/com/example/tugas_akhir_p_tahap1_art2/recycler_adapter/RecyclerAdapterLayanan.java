...
import com.example.tugas_akhir_p_tahap1_art2.halaman_utama.AlQuranActivity;
import com.example.tugas_akhir_p_tahap1_art2.halaman_utama.HistoryOrderActivity;
import com.example.tugas_akhir_p_tahap1_art2.halaman_utama.lokasiactivity;
...
public class RecyclerAdapterLayanan extends RecyclerView.Adapter<RecyclerAdapterLayanan.MyAdapter> {
    ...
    public void onBindViewHolder(@NonNull final RecyclerAdapterLayanan.MyAdapter holder, int position) {
        ...
        if (modelDashboard.getNamaLayanan()=="Al Quran"){
            holder.cardLayanan.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    View dialogView = LayoutInflater.from(v.getRootView().getContext()).inflate(R.layout.activity_kitab, null);
        ...
        if (modelDashboard.getNamaLayanan()=="Lokasi"){

            holder.cardLayanan.setOnClickListener(new View.OnClickListener() {
                @Override
                public void onClick(View v) {
                    View dialogView = LayoutInflater.from(v.getRootView().getContext()).inflate(R.layout.activity_lokasi, null);
                    Intent intent = new Intent(v.getContext(), lokasiactivity.class);
                    v.getContext().startActivity(intent);
                }
